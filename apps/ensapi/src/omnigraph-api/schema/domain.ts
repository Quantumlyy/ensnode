...
import { resolveGrailsForDomain } from "@/omnigraph-api/lib/resolve-grails-field";
...
DomainInterfaceRef.implement({
  ...
  fields: (t) => ({
    ...
    listings: t.field({
      ...
      nullable: true,
      resolve: (parent) =>
        resolveGrailsForDomain(parent.canonicalName, (client, name) => client.listingsByName(name)),
    }),
    ...
    offers: t.field({
      ...
      nullable: true,
      resolve: (parent) =>
        resolveGrailsForDomain(parent.canonicalName, (client, name) => client.offersByName(name)),
    }),
    ...
    floorListing: t.field({
      ...
      nullable: true,
      resolve: (parent) =>
        resolveGrailsForDomain(parent.canonicalName, async (client, name) => {
          const listings = await client.listingsByName(name);
          if (listings.length === 0) return null;
          return listings.reduce((min, listing) =>
            listing.priceWei < min.priceWei ? listing : min,
          );
        }),
    }),
  }),
